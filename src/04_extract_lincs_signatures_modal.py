...
app = modal.App("extract-lincs-signatures")
...
def extract_signatures():
    ...
    for label, brd_list in targets.items():
        ...
        for N in [100, 300, 500]:
            ...
            results[f"{label}_up_{N}"] = "\n".join(up)
            results[f"{label}_down_{N}"] = "\n".join(down)
...
def main():
    import os
    print("Extracting signatures via Modal...")
    res = extract_signatures.remote()
    ...
    for key, content in res.items():
        with open(os.path.join(out_dir, f"{key}.txt"), "w") as f:
            f.write(content + "\n")
    print(f"Signatures successfully saved to {out_dir}")